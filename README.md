...
> 真实 60 天模型输入 → 本地 PyTorch / Mamba 推理 → RUL / 年化退化率 → 基于退化率的趋势外推 → 前端可视化

## 当前已经能做什么

- 内置 PVDAQ 1403 的 740 个真实 60 天窗口，可在前端选择并实时推理。
- 使用 `PV_RUL_Mamba PEFT-1403` 完整权重，不是写死结果。
- 输出：RUL 参考值、年化退化率、当前 Pmax、健康状态；页面可附加“基于年化退化率的未来60天趋势外推”。
- 展示 6 个输入特征的 60 天曲线。
...
4. 模型内部 `trajectory_head` 保留用于训练期物理/平滑/一致性约束，V1 不再把它展示为“未来60天直接预测”。
5. 客户快照中的双通道为重复副本，演示固定使用第1路，不再提供无效通道切换。
6. V1 不重新训练模型；训练原始数据并未完整包含在客户快照中。
...
- `trend_projection_60d`：根据 `current_pmax_ratio` 与 `rate_pct_per_year` 计算的 60 天线性趋势外推，仅作解释性展示，**不是 Mamba 对未来逐日值的直接预测**
- `input_pmax_history` / `input_rows`：本次实际送入模型的历史输入，用于保证内置样本和上传 CSV 的图表都与本次预测一致
